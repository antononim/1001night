        ...
        if st.button(
            ...
            if materials is not None:
                ...
                try:
                    ...
                    st.warning(
                        f"Не удалось структурировать результаты через Gemini: {exc}"
                    )
                st.session_state.meeting_analysis = analysis_text or summary_text
                if not st.session_state.notes_input.strip() and summary_text:
                    st.session_state.notes_input = summary_text
...
if run_button:
    ...
    if meeting_materials is None and audio_bytes:
        ...
        summary_text = meeting_materials.summary or transcript_text
        st.session_state.manual_transcript = transcript_text
        st.session_state.transcript_summary = summary_text
        st.session_state.meeting_analysis = summary_text
        if not notes_text and summary_text:
            notes_text = summary_text.strip()
    ...
    agent_labels = {agent_id: AGENT_REGISTRY[agent_id].title for agent_id in selected_agents}
...
analysis_output = st.session_state.get("meeting_analysis", "")
...
with materials_tab:
    ...
    if analysis_output:
        try:
            ...
            st.warning(f"Не удалось обработать текст саммари: {exc}")
        else:
