...
def analysis(state: Dict[str, Any]) -> Dict[str, Any]:
    ...
    artifact = save_artifact(run_id, "finance_analysis.md", response)

    board = deepcopy(state.get("board", {}))
    board.setdefault("finance_analysis", {})
    board["finance_analysis"].update({"status": "Done", "notes": response})
